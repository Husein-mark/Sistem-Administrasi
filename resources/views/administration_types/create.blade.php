<x-layouts.app title="Tambah Jenis Layanan Administrasi — SIAKAD Online">
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            ...
            <p class="text-xs sm:text-sm text-slate-500 mt-1">
                Daftarkan jenis surat administrasi baru yang dapat dipilih oleh peserta didik.
            </p>
        </div>

        <div class="siakad-card bg-white rounded-2xl p-6 sm:p-8 border border-slate-200 shadow-sm">
            <form method="POST" action="{{ route('administration-types.store') }}" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <input type="text" id="nama_layanan" name="nama_layanan" value="{{ old('nama_layanan') }}" required placeholder="Contoh: Surat Pengantar Beasiswa"
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" id="kode_layanan" name="kode_layanan" value="{{ old('kode_layanan') }}" required placeholder="Contoh: SK-BEASISWA"
                    ...
                    <div>
                        ...
                        <input type="number" id="estimasi_hari" name="estimasi_hari" value="{{ old('estimasi_hari', 1) }}" min="1" max="60" required
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1B6CF2] focus:border-[#1B6CF2]">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea id="deskripsi" name="deskripsi" rows="3" placeholder="Jelaskan kegunaan dan fungsi surat ini..."
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1B6CF2] focus:border-[#1B6CF2]">{{ old('deskripsi') }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="persyaratan" name="persyaratan" rows="3" placeholder="Sebutkan berkas yang perlu disiapkan siswa..."
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-medium text-slate-800 focus:outline-none focus:ring-2 focus:ring-[#1B6CF2] focus:border-[#1B6CF2]">{{ old('persyaratan') }}</textarea>
                </div>

                <div class="pt-1">
                    <label class="flex items-center gap-2.5 cursor-pointer text-sm font-semibold text-slate-700">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-slate-300 text-[#1B6CF2] focus:ring-[#1B6CF2]">
